...
Spigot APIを用い、ゲーム内でのイベントを起点とし、スコア加算処理やデータベースへの保存機能を設計・実装しています。
<br/> 
<br/> 
...
## 📝 工夫・苦戦した点 まとめ
◇ 鉱石ごとの情報管理  
列挙型を用い、if文などの条件分岐を増やさないよう意識しました。<br/> 
後から情報を追加する場合でも対応しやすく拡張性を持たせた設計にしました。

◇ 地形復元  
ゲーム終了後に、記録しておいた情報を元に、自動で地形復元することで、地形への影響を最小限に抑え繰り返しプレイ可能な設計にしました。

◇ 時間管理  
スケジューラーを用いて「ゲーム開始前カウントダウン」「ゲーム本編（制限時間管理）」「終了処理」の3段階に分けて管理しました。<br/> 
当初は、３段階の処理を一つのタイマー内で扱ってしまい、時間表示のタイミングがずれたり、処理が止まらないなどの不具合が発生しましたが、修正と動作確認を繰り返し実行し、各フェーズ毎に処理を分離し設計し直すことで解決することができました。<br/> 
これにより、処理の見通しを良くし拡張や修正を行いやすい構成となりました。

◇ 全体を通して  
メソッド抽出や変数名の命名、定数化、リファクタリングを行い、機能実装だけでなく少しでもコードの可読性が上がるよう意識して作成しました。
...
## 🪏 今後実装予定の機能  
- マルチプレイ対応（複数のプレイヤーが同時参加できるよう拡張）
- 難易度設定（easy / normal / hard を設定し、難易度によって制限時間やゲーム条件を切り替える設定へ）
- データベースを用いたスコアのランキング表示（保存されたスコア履歴の中から上位３件だけを取得して表示する）
<br/> 
...
また、ご意見やお気づきの点がございましたら、 Xアカウント（[@nap_7878](https://x.com/nap_7878)）までご連絡いただけましたら幸いです。  
